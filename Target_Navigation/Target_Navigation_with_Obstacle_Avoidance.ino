...
void loop() {
        ...
        if ((millis() - publish_sensor_time) >= (1000 / SENSOR_PUBLISH_RATE))
              ...
              if(GPS.fix!=0){
                     ...
                     if(at_goal()){
                          ...
                          while(1){
                           ...
                           if(front==true){ // front sensor detects obstacle
                                     nh.loginfo("GOUND OBSTACLE AHEAD!");
                                     nh.loginfo("In obstacle following mode.");
                                     cmdvelCb(0.3,-3);
                            }
                            else if(front==false && right==true){ // only right sensor detects obstacle
