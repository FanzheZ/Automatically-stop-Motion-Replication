 ...
 if (Serial.available()) {
   ...
   // Serial.println(Serial.read());
    delay(1000);
     ...
     if(distancef>50&&distanceb>50&&distancel>50&&distancer>50){
        parseInput(FORWARD);
        Serial.println("F");
        Serial.println();
        Serial.println();
      ...
      // when front and back sensors detect object, stop.
      if ((distancef<=50 && distancef>0)||(distanceb<=50 && distanceb>0)){
        parseInput(STOP);
        Serial.println("S");
      }
    
     // when left sensor detect object, turn right
     if (distancel<=50&& distancel>0){
         if ((distancef<=50 && distancef>0)||(distanceb<=50 && distanceb>0))
         {
            parseInput(STOP);
            Serial.println("S");
     ...
     // when right sensor detect object, turn left
     if (distancer<=50&& distancer>0)
     {
        if ((distancef<=50 && distancef>0)||(distanceb<=50 && distanceb>0))
        {
            parseInput(STOP);
